...
;(function(seajs, util, config) {
  ...
  Module.prototype._use = function(ids, callback) {
	...
	if( util.isString(ids) && !util.isFunction(callback) ) {
		if( util.isArray(callback) ) {
			//
			// Support syntax: seajs.use('[moduleid].[function name]', [parameters]);
			//
			var names = ids.split('.');
			if( names.length == 2 ) {
				this._use(names[0], function(module){ module[names[1]].apply(module,callback); })
			}
      return;
		}
			...
			//
			//	Obsoleted:
			// 		Support a simple syntax: seajs.use('[module id]', { [function name] : [parameters] } );
			//			For example: seajs.use('pctx', { mix: { 'subject': 'kuainv'} });
			//
			for( var key in callback ) {
				(function(s,key){
			        s._use(ids, function(module){
			          module[key].apply(module,callback[key]);
			  			});
					})(this,key);
			}
      return;
  ...
  Module.prototype._load = function(uris, callback) {
    ...
    function cb(module) {
      if( module && module.status < STATUS.READY ) {
  ...
  function resolve(ids, refUri) {
    ...
    return util.map(ids, function(id) {
      if(util.isAbsolute(id)){
        config.base = util.dirname(id); //从入口JS中解析base url
      }
  ...
  seajs.config = function(o) {
	...
	if( o.modules ) {
		...
		if(!config.modules)
			config.modules = [];
		if( o.modules )
			config.modules = config.modules.concat(o.modules);

        util.forEach(o.modules, function(item) {
        	var alias = item[0];
        	var version = item[1];
        	if( alias != 'seajs' ){
        		o.alias[alias] = o.base + alias + '/' + version + '/' + alias + '.js';
        	}
          });
	}
    ...
    // Rebuild mappings
    if( config.modules ) {
	    util.forEach(config.modules, function(item) {
	    	// Map alias url to package alias url. The item = [alias, version, package alias]
	    	if( item[2] && config.alias[item[2]]) {
	    		if( !config.map ) {
	    			config.map = [];
	    		}
	    		config.map.push(
